...
! PID controller entry point. Wires the shared input/mode/state modules to the
! PID control law and runs the UDP control loop against the physics sim.
program main
  ...
  if (command_argument_count() < 1) then
    ...
    write(*,'(A)') "Usage: PID <config.json>"
    stop 1
  end if
  call get_command_argument(1, filename)
  call load_pid_config(trim(filename), cfg, pm, ctrl)

  allocate(instate(cfg%base%pmap%n))
  instate = 0.0

  call law_init(law, cfg, pm)

  ! physics socket (blocking with timeout - syncs to physics timestep)
  ...
  first_packet = .true.
  print *, "=========================================="
  print *, "PID controller ready - entering main loop"
  print *, "=========================================="
  ...
  do
    ...
    if (got_data) then
      ...
      if (real(clock_now - clock_last_packet) / real(clock_rate) > IDLE_TIMEOUT) then
        print *, ""
        print *, "No UDP packets received for 10 seconds. Shutting down."
        exit
      end if
      cycle
    end if
    ...
    if (cfg%base%pilot_enabled) then
      if (trim(cfg%base%pilot_type) == 'csv') then
        ...
        call law_sync_bumpless(law, pm, st, ctrl, trans)
        call mode_apply_sticks(gp, pm, dt)
      end if
    end if

    ! run the control law
    call law_step(law, pm, st, dt, ctrl)

    ! send controls (entity-tagged) + HUD status
...
contains
  ...
  subroutine send_hud()
    integer :: m
    m = effective_mode(pm)
    hud_status = [ merge(1.0, 0.0, m >= MODE_RATES), &     ! p augmentation
                   merge(1.0, 0.0, m >= MODE_RATES), &     ! q augmentation
                   merge(1.0, 0.0, m >= MODE_RATES), &     ! r augmentation
                   merge(1.0, 0.0, pm%velocity_on), &      ! velocity hold
                   merge(1.0, 0.0, m >= MODE_ANGLES), &    ! bank hold
                   merge(1.0, 0.0, m >= MODE_ANGLES), &    ! pitch hold
                   merge(1.0, 0.0, pm%beta_on .and. m >= MODE_ANGLES), &  ! sideslip
                   merge(1.0, 0.0, m == MODE_ALTITUDE), &  ! altitude hold
  ...
  subroutine print_status()
    ...
    write(*,'(A,L1)') "  sideslip hold:  ", pm%beta_on
    if (effective_mode(pm) == MODE_ALTITUDE) &
      write(*,'(A,F8.1,A)') "    h_cmd:", pm%h_cmd, " ft"
    print *, "======================================="
