#!/usr/bin/env zsh
# Linux/macOS build for the PID controller. Builds the shared common/ modules
# first, then the controller-specific modules, linking ../common/obj/*.o.
...
if [[ "$MODE" == "debug" ]]; then
  echo "Building PID controller [DEBUG]..."
  ...
  echo "Building PID controller [RELEASE]..."
  FFLAGS=(-c "${COMMON[@]}" -O2 -I "$OBJ_DIR" -I "$COMMON_OBJ" -J "$OBJ_DIR")
  LFLAGS=("${COMMON[@]}" -O2 -static)
fi

SRCS=(config_m.f90 control_law_m.f90 controller_main.f90)
